...
function createWindow() {
  win = new BrowserWindow({
    icon: path.join(process.env.VITE_PUBLIC || "", "something"),
    // Change icon path
    width: 800,
    height: 600,
    webPreferences: {
  ...
  if (app.isPackaged) {
    ...
    autoUpdater.on("error", (err: any) => {
      // Use any to handle various error shapes
      log.error("Error in auto-updater:", err); // Log the full error object
      const message =
        err?.message ?? JSON.stringify(err ?? "Unknown update error");
    ...
    ipcMain.on("start-download", () => {
      ...
      autoUpdater.downloadUpdate().catch((err: any) => {
        ...
        const message =
          err?.message ?? JSON.stringify(err ?? "Unknown download error");
